...
Originally used only to fetch each device's devicekey for the LAN AES
protocol. Also used for the POWCT's live power/energy polling (see
ewelink_power.py) and, via the websocket dispatch channel below, to pull
the device's rolling 31-day hourly-kWh history (getHoursKwh/hoursKwhData) —
an undocumented capability, not present in the public v2 REST API, found
by inspecting this device's own params dict (see get_hours_kwh).
